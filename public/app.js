(() => {
  ...
  // Reproductor híbrido: para temas de YouTube usa el reproductor IFrame de YouTube (el
  // audio lo entrega el navegador, sin streaming del servidor, sin cookies ni API key);
  // para archivos locales (/dl/) usa el <audio> real. El backend solo hace búsqueda,
  // Auto-DJ y lyrics. Así suena en cualquier host gratis (p.ej. Render) sin tu PC.
  ...
  const armStall = (track) => {
    ...
    if (audio.__isShim && audio.mode === 'yt') return; // el IFrame gestiona su propio buffering
    clearStall();
...
updatePlaylistCount();
  ...
  let endWatchAt = 0;
  const warmNext = () => {
    if (audio.__isShim && audio.mode === 'yt') { warmNextFor = -1; return; } // el IFrame carga el siguiente solo
  ...
  const playFrom = (index, src = activeSrc, manual = false) => {
    ...
    if (false && !playingFile && !nativeMode && !useStream) {
      ...
      liveBadge.classList.toggle('hidden', true);
      durBadge.textContent = track.duration ? fmt(track.duration) : '';
      prepareLyrics(track); warmNextFor = -1; updateAmbient(track);
      return;
    }
    armStall(track);
  ...
  const buildGraph = () => {
    ...
    if (audio.__isShim && audio.mode === 'yt') return; // IFrame: sin Web Audio (la onda es simulada)
    try {
      const el = (audio.__isShim && audio.real) ? audio.real : audio;
...
const fuzzyCatalog = (query, limit = 8) => {
  ...
  audio.addEventListener('stalled', () => {
    ...
    if (audio.__isShim && audio.mode === 'yt') return; // el IFrame gestiona su buffering
