...
const CLIENT_CHAIN = (process.env.YTDLP_CLIENTS || 'android,tv,ios,web_embedded')
...
async function resolveStream(videoId, { clients = CLIENT_CHAIN, force = false } = {}) {
  ...
  if (order.length > 1 && !force) {
    ...
    const preferred = ['mweb', 'android', 'web_embedded', 'wyvern', 'ios', 'tv'];
...
app.get('/api/ping', (req, res) => {
  res.json({
    ...
    cookies: cookieArgs().length ? 'sí' : 'no',
  });
});
...
app.get('/api/img', async (req, res) => {
  ...
  if (!/^https?:\/\//i.test(u)) return res.status(400).json({ error: 'url inválida' });
  try {
...
// ---- Auto-DJ: dada una semilla (artista o consulta), arma una colección
...
app.get('/api/autodj', async (req, res) => {
  ...
  if (!base) return res.status(400).json({ error: 'Semilla vacía (usa artist o seed)' });
  try {
    let out = await djCollect(base);
    // sumar el catálogo del artista para mantener su estilo y época
    if (artist) {
      try {
...
// ---- expansión estilo DJ senior: interpreta palabras clave y combina búsquedas ----
...
app.get('/api/search', async (req, res) => {
  ...
  const collName = type === 'playlist' ? `Colección DJ · ${q}` : '';
  const ql = norm(q);
  try {
    let out = [];
    if (type === 'artist') {
    ...
    // responde (esperamos máximo 1.2s; si tarda más, la resolución queda en vuelo y el
    // cliente la recoge vía el mismo getPlayableStream en /api/stream)
    if (out.length) {
      await Promise.race([
        getPlayableStream(out[0].id).catch(() => {}),
        new Promise((r) => setTimeout(() => r(null), 1200)),
...
async function fetchPlaylistTracks(playlistId) {
  ...
  if (!j) throw new Error('La lista no respondió en formato JSON');
...
app.get('/api/playlist', async (req, res) => {
  ...
  if (!playlistId) return res.status(400).json({ error: 'URL de lista de reproducción inválida' });
  try {
    const { name, results } = await fetchPlaylistTracks(playlistId);
    if (!results.length) return res.status(404).json({ error: 'La lista no devolvió canciones' });
    // pre-resolver el primer stream para arranque rápido (mismo truco que /api/search)
...
app.get('/api/stream/:id', async (req, res) => {
  ...
  try {
    ...
    // ---- modo 1 (direct): redirección 302 al stream de googlevideo ----
    ...
    // Descubrir tamaño total + tipo del stream (probe 0-0, siempre aceptado).
    const probe = await fetch(url, { headers: { Range: 'bytes=0-0' }, redirect: 'follow' });
    if (!probe.ok) {
      streamCache.delete(videoId);
      return res.status(502).json({ error: 'El origen no respondió correctamente' });
    }
    ...
    // cliente a rangos cerrados con límite explícito y servimos por chunks.
    ...
    if (!total) {
      // Sin tamaño conocido: servimos lo que el upstream devuelva (fallback directo).
    ...
    // Responder 206 con el rango pedido (límite cerrado) + headers correctos.
    res.status(206);
    ...
    // (especialmente sensible al hacer seek o al iniciar la reproducción).
    const fetchChunk = (from, to) =>
      fetch(url, { headers: { Range: `bytes=${from}-${to}` }, redirect: 'follow' })
        .then((r) => { if (!r.ok || !r.body) throw new Error('chunk ' + from + ' falló: ' + r.status); return r; });
...
app.listen(PORT, () => {
  ...
  console.log(`  Motor: búsqueda web + audio puro (sin anuncios) · ffmpeg: ${FFMPEG ? 'OK' : 'NO'}`);
  console.log(`  Descargas: ${DOWNLOAD_DIR}`);
  startDiscovery();
  console.log('');
});

// ---- auto-detección para la app Android (UDP broadcast por la red local) ----
